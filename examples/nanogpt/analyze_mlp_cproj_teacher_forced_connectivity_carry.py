#!/usr/bin/env python3
"""Replay fixed-coordinate c_proj carry with wider matching connectivity."""
...
ARMS = (
    Arm("neighbors64_full_carry", 64),
    Arm("neighbors128_full_carry", 128),
    Arm("neighbors256_full_carry", 256),
)
CONTROL = ARMS[0].name
...
def aggregate_rows(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for candidate in CANDIDATE_ORDER:
        ...
        recovery_ratios = {
            ...
            / max(
                float(
                    scores[f"{CONTROL}@{step}"][
                        "aggregate_endpoint_recovery"
                    ]
                ),
                1e-30,
            )
        ...
        comparison = {
            ...
            "terminal_endpoint_error_energy_ratio": endpoint_error_ratio,
    ...
    return {
        ...
        "decision": (
            ...
            else "RETAIN_NEIGHBORS64_AND_CLOSE_RADIUS_BRANCH"
        ),
    }
...
def main() -> None:
    ...
    if plan.get("schema_version") != (
        "mai_124m_mlp_cproj_teacher_forced_connectivity_carry_plan_v1"
    ):
    ...
    cells_path = args.output / "cproj_connectivity_carry_cells.csv"
    aggregate_path = args.output / "cproj_connectivity_carry_result.json"
    write_csv(cells_path, rows)
    aggregate_path.write_text(json.dumps(aggregate, indent=2, sort_keys=True) + "\n")
    script = Path(__file__).resolve()
    metadata = {
        "schema_version": "nanogpt_cproj_teacher_forced_connectivity_carry_v1",
    ...
    metadata_path = args.output / "cproj_connectivity_carry_metadata.json"
