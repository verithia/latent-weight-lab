#!/usr/bin/env python3
"""Test cross-batch task-gradient consensus for c_proj output pair selection."""
...
SCHEMA_VERSION = "mai_124m_mlp_cproj_cross_batch_consensus_selector_v1"
EXPECTED_PLAN_SCHEMA = "mai_124m_mlp_cproj_cross_batch_consensus_selector_plan_v1"
ARMS = (
    "frobenius_output32",
    "fit_task_output32",
    "consensus_task_output32",
)
WINDOWS = ("fit", "holdout")
...
def validate_plan(plan: dict[str, Any]) -> None:
    ...
    if observed != expected:
        raise ValueError("consensus plan does not match the immutable v1 contract")
    if plan.get("authorization", {}).get("implement_and_run_zero_update_analysis") is not True:
        raise ValueError("zero-update consensus analysis is not authorized")


def normalized_consensus_gradient(
    recorded: torch.Tensor, fit: torch.Tensor
) -> tuple[torch.Tensor, dict[str, float]]:
    if recorded.shape != fit.shape or recorded.ndim != 2:
        raise ValueError("consensus gradients must share one matrix shape")
    recorded = recorded.float()
    fit = fit.float()
    recorded_norm = recorded.norm().clamp_min(1e-30)
    fit_norm = fit.norm().clamp_min(1e-30)
    recorded_unit = recorded / recorded_norm
    fit_unit = fit / fit_norm
    consensus = 0.5 * (recorded_unit + fit_unit)
    cosine = float((recorded_unit * fit_unit).sum())
    return consensus, {
        "recorded_gradient_fro": float(recorded_norm),
        "fit_gradient_fro": float(fit_norm),
        "recorded_fit_gradient_cosine": cosine,
        "consensus_fro": float(consensus.norm()),
    }
...
def aggregate_results(
    ...
    fit_arm = by_arm["fit_task_output32"]
    candidate = by_arm["consensus_task_output32"]
    task = {
        window: {
            "control": float(control["task_gradient_predicted_ce_decrease"][window]),
            "fit_task": float(fit_arm["task_gradient_predicted_ce_decrease"][window]),
            "consensus": float(candidate["task_gradient_predicted_ce_decrease"][window]),
        }
        for window in WINDOWS
    }
    advantages = {
        window: task[window]["consensus"] - task[window]["control"]
        for window in WINDOWS
    }
    retention = (
            ...
            comparisons.append(
                {
                    ...
                    "fit_task_loss": indexed["fit_task_output32"],
                    "consensus_loss": indexed["consensus_task_output32"],
                    "consensus_minus_frobenius": (
                        indexed["consensus_task_output32"]
                        - indexed["frobenius_output32"]
                    ),
                    "consensus_minus_fit_task": (
                        indexed["consensus_task_output32"]
                        - indexed["fit_task_output32"]
                    ),
                }
            )
    control_wins = sum(
        float(row["consensus_minus_frobenius"]) < 0.0 for row in comparisons
    )
    holdout_control_wins = sum(
        float(row["consensus_minus_frobenius"]) < 0.0
        for row in comparisons
        if row["window"] == "holdout"
    )
    holdout_fit_wins = sum(
        float(row["consensus_minus_fit_task"]) < 0.0
        for row in comparisons
        if row["window"] == "holdout"
    )
    ...
    mean_fit = sum(float(row["fit_task_loss"]) for row in comparisons) / len(
        comparisons
    )
    mean_candidate = sum(float(row["consensus_loss"]) for row in comparisons) / len(
        comparisons
    )
    ...
    gate = {
        ...
        "holdout_task_descent_positive_and_above_frobenius": (
            task["holdout"]["consensus"] > 0.0
        ...
        "holdout_task_descent_at_least_fit_task_arm": (
            task["holdout"]["consensus"] >= task["holdout"]["fit_task"]
        ),
        ...
        "all_4_holdout_ce_wins_vs_frobenius": holdout_control_wins == 4,
        "at_least_7_of_8_ce_wins_vs_frobenius": control_wins >= 7,
        "at_least_3_of_4_holdout_ce_wins_vs_fit_task": holdout_fit_wins >= 3,
        "mean_ce_at_least_0p0005_better_than_frobenius": (
            mean_candidate <= mean_control - 0.0005
        ),
        "mean_ce_no_worse_than_fit_task": mean_candidate <= mean_fit,
    }
    passed = all(gate.values())
    return {
        "by_arm": by_arm,
        "task_gradient_predicted_ce_decrease": task,
        "task_descent_advantage_vs_frobenius": advantages,
        ...
        "consensus_gradient": {
            "cells": len(consensus_rows),
            "mean_recorded_fit_cosine": sum(
                float(row["recorded_fit_gradient_cosine"])
                for row in consensus_rows
            )
            / len(consensus_rows),
            "minimum_recorded_fit_cosine": min(
                float(row["recorded_fit_gradient_cosine"])
                for row in consensus_rows
            ),
        },
        "finite_step": {
            "comparisons": comparisons,
            "wins_vs_frobenius": control_wins,
            "holdout_wins_vs_frobenius": holdout_control_wins,
            "holdout_wins_vs_fit_task": holdout_fit_wins,
            "mean_frobenius_loss": mean_control,
            "mean_fit_task_loss": mean_fit,
            "mean_consensus_loss": mean_candidate,
        },
        "gate": gate,
        "passed": passed,
        "decision": (
            "CROSS_BATCH_CONSENSUS_SELECTOR_PASS"
            if passed
            else "REJECT_CROSS_BATCH_CONSENSUS_SELECTOR"
        ),
        "authorization": {
...
def main() -> None:
    ...
    if (
        file_sha256(Path(plan["identity"]["sequential_trust_result"]))
        != plan["identity"]["sequential_trust_result_sha256"]
        ...
        raise ValueError("consensus plan input identity mismatch")
    ...
    for phase_index, (phase_start, phase_end) in enumerate(phases):
        ...
        for layer in layers:
            ...
            fit_t, fit_diagnostics = fit_task_gradient_hybrid_pass(
                source,
                residual_t,
            ...
            consensus_gradient, consensus_diagnostics = normalized_consensus_gradient(
                state["gradient_after_clip"].to(args.device),
                gradients["fit"][layer].to(args.device),
            )
            consensus_t, candidate_diagnostics = fit_task_gradient_hybrid_pass(
                source,
                residual_t,
                consensus_gradient.T.contiguous(),
                stages=int(chart["output_stages"]),
                neighbors=int(chart["neighbors"]),
                seed=seed + 2,
            )
            ...
            final_weights = {
                ...
                "fit_task_output32": fit_t.T.contiguous() * decay,
                "consensus_task_output32": consensus_t.T.contiguous() * decay,
            }
            ...
            for pass_name, diagnostics in (
                ...
                ("fit_task_output32", fit_diagnostics),
                ("consensus_task_output32", candidate_diagnostics),
            ):
                consensus_rows[-1][f"{pass_name}_maximum_abs_angle"] = float(
    ...
    aggregate = aggregate_results(metric_rows, finite_rows, consensus_rows)
    args.output.mkdir(parents=True, exist_ok=True)
    metrics_path = args.output / "consensus_selector_cells.csv"
    finite_path = args.output / "consensus_selector_finite_ce.csv"
    consensus_path = args.output / "consensus_selector_gradients.csv"
    ...
    result = {
        ...
        "artifacts": {
            ...
            "gradients": {"path": str(consensus_path), "sha256": file_sha256(consensus_path)},
        },
        "aggregate": aggregate,
        "elapsed_seconds": time.time() - started,
    }
    result_path = args.output / "consensus_selector_result.json"
