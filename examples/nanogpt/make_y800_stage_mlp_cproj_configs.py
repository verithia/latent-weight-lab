...
BASE = {
    ...
    "dtype": "bfloat16",
    "compile": False,
    "block_fht_targets": ["attn.c_proj", "mlp.c_fc", "mlp.c_proj"],
    "block_fht_latent_ratio": 0.08,
    "block_fht_latent_ratios": {"attn.c_proj": 0.08, "mlp.c_fc": 0.10, "mlp.c_proj": 0.08},
...
def main() -> None:
    specs = [
        (
            "y800_stage3_100m_01_cproj_cfc_l10_lr32_mproj_l5",
            {"block_fht_latent_ratios": {"attn.c_proj": 0.08, "mlp.c_fc": 0.10, "mlp.c_proj": 0.05}},
        ),
        (
            "y800_stage3_100m_02_cproj_cfc_l10_lr32_mproj_l8",
            {"block_fht_latent_ratios": {"attn.c_proj": 0.08, "mlp.c_fc": 0.10, "mlp.c_proj": 0.08}},
        ),
        (
            "y800_stage3_100m_03_cproj_cfc_l10_lr32_mproj_l10",
            {"block_fht_latent_ratios": {"attn.c_proj": 0.08, "mlp.c_fc": 0.10, "mlp.c_proj": 0.10}},
        ),
        (
            "y800_stage3_100m_04_cproj_cfc_l10_lr32_mproj_gain",
            {
                "block_fht_latent_ratios": {"attn.c_proj": 0.08, "mlp.c_fc": 0.10, "mlp.c_proj": 0.08},
                "block_fht_output_gain_targets": ["mlp.c_proj"],
                "block_fht_cache_weights": False,
            },
        ),
    ]
    tasks = [(name, write(name, **updates)) for name, updates in specs]
    queue_path = CONFIG_DIR / "y800_stage_mlp_cproj_queue.tsv"
