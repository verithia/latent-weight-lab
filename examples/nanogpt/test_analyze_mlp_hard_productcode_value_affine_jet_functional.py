...
PLAN = REPO_ROOT / "examples/nanogpt/configs/selection_artifacts/124m_mlp_hard_productcode_value_affine_jet_functional_plan.json"


def test_direct_entrypoint_resolves_repository_package() -> None:
    script = Path(__file__).with_name(
        "analyze_mlp_hard_productcode_value_affine_jet_functional.py"
    )
...
def test_exact_h61_accounting_matches_plan() -> None:
    ...
    assert accounting["extra_mlp_matmul_fraction"] == 0.0625


def tiny_student() -> HardProductCodeValueAffineJet:
    ...
    return HardProductCodeValueAffineJet(
        detector,
        write,
        [0],
        anchors,
...
def test_hard_function_analytic_jvp_matches_autograd() -> None:
...
def test_hard_value_branch_has_exact_parent_jvp() -> None:
    student = tiny_student()
    student.eval()
    with torch.no_grad():
        student.codebook.normal_()
    inputs = torch.randn(5, 4, generator=torch.Generator().manual_seed(19))
    directions = torch.randn(5, 4, generator=torch.Generator().manual_seed(23))
...
def test_straight_through_router_gradient_and_compact_artifact() -> None:
    student = tiny_student()
    student.train()
    inputs = torch.randn(5, 4, generator=torch.Generator().manual_seed(29))
    with torch.no_grad():
        student.codebook.normal_()
    output, _ = student.forward_function(0, inputs, None)
    output.square().mean().backward()
    assert student.router.grad is not None
    assert float(student.router.grad.norm()) > 0
    assert student.codebook.grad is not None
    accounting = deployment_accounting(
    ...
    assert artifact["codebook"].dtype == torch.float16
