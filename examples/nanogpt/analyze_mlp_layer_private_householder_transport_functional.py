#!/usr/bin/env python3
"""Frozen H65 layer-private Householder MLP functional audit."""
...
SCHEMA_VERSION = "nanogpt_mlp_layer_private_householder_transport_functional_v1"
PLAN_SCHEMA_VERSION = (
    "nanogpt_mlp_layer_private_householder_transport_functional_plan_v1"
)
CANDIDATE_NAME = "learned_layer_private_householder_transport_atlas"
...
def apply_householder_product(
    ...
    *,
    ...
    for index in range(count):
        ...
        denominator = vector.square().sum().clamp_min(epsilon)
        coefficient = (result @ vector / denominator).unsqueeze(-1)
...
def deployment_accounting(
    *,
    ...
    householder = layers * 2 * reflectors_per_side * width
    ...
    return {
        ...
        "fp16_householder_values": householder,
        ...
        / (8 * width * width),
        "extra_householder_operation_fraction": (
            2 * 2 * reflectors_per_side * width
        )
        / (8 * width * width),
...
class LayerPrivateHouseholderTransport(HardProductCodeValueAffineJet):
    def __init__(
        self,
        ...
        *,
        ...
        router_rank: int,
        tangent_rank: int,
        reflectors_per_side: int,
        router_q_seed: int,
        router_p_seed: int,
        tangent_v_seed: int,
        input_reflector_seed: int,
        output_reflector_seed: int,
        code_gain_initial: float,
        router_bias_initial: float,
        temperature: float,
        layer_diagonal_initial: float,
        reflector_epsilon: float,
        ...
        self.reflectors_per_side = int(reflectors_per_side)
        self.reflector_epsilon = float(reflector_epsilon)
        ...
        shape = (len(layers), self.reflectors_per_side, self.width)
        self.input_reflectors = torch.nn.Parameter(
            deterministic_standard_normal(*shape, seed=input_reflector_seed)
        )
        self.output_reflectors = torch.nn.Parameter(
            deterministic_standard_normal(*shape, seed=output_reflector_seed)
        )
    ...
    def _parts(self, row: int, mode: str) -> dict[str, Any]:
        parts: dict[str, Any] = {
            ...
            "input_reflectors": self.input_reflectors[row],
            "output_reflectors": self.output_reflectors[row],
            "input_count": self.reflectors_per_side,
            "output_count": self.reflectors_per_side,
        ...
        if mode == "step_zero_parent":
            ...
            parts["input_count"] = 0
            parts["output_count"] = 0
        elif mode == "input_transport_only":
            parts["output_count"] = 0
        elif mode == "output_transport_only":
            parts["input_count"] = 0
        elif mode == "one_reflector_per_side":
            parts["input_count"] = 1
            parts["output_count"] = 1
        elif mode == "shared_across_layers":
            parts["input_reflectors"] = self.input_reflectors.mean(dim=0)
            parts["output_reflectors"] = self.output_reflectors.mean(dim=0)
            ...
            raise ValueError(f"unknown H65 mode: {mode}")
        return parts
    ...
    def forward_function(
        ...
        *,
        ...
        transported_input = apply_householder_product(
            inputs - p["anchor"],
            p["input_reflectors"],
            count=int(p["input_count"]),
            epsilon=self.reflector_epsilon,
        )
        latent = (transported_input @ p["tangent_v"]) * p["layer_diagonal"]
        raw_tangent = latent @ p["tangent_u"].T
        tangent_correction = apply_householder_product(
            raw_tangent,
            p["output_reflectors"],
            count=int(p["output_count"]),
            epsilon=self.reflector_epsilon,
        )
        ...
        transported_direction = apply_householder_product(
            directions,
            p["input_reflectors"],
            count=int(p["input_count"]),
            epsilon=self.reflector_epsilon,
        ...
        tangent_action = apply_householder_product(
            raw_action,
            p["output_reflectors"],
            count=int(p["output_count"]),
            epsilon=self.reflector_epsilon,
...
def optimizer_for(
    student: LayerPrivateHouseholderTransport, plan: dict[str, Any]
...
def fit_student(
    student: LayerPrivateHouseholderTransport,
    bank: FunctionalBank,
    ...
    *,
    ...
    for iteration in range(iterations):
        ...
        if action is None:
            raise RuntimeError("H65 student JVP is missing")
        output_loss = (
...
def terminal_artifact(
    student: LayerPrivateHouseholderTransport, accounting: dict[str, Any]
) -> dict[str, Any]:
    values = {
        ...
        "input_reflectors": student.input_reflectors.detach().to(torch.float16).cpu(),
        "output_reflectors": student.output_reflectors.detach().to(torch.float16).cpu(),
    ...
    return {
        "schema_version": "layer_private_householder_checkpoint_v1",
        **values,
        "layers": student.layers,
        "banks": student.banks,
        "codes_per_bank": student.codes_per_bank,
        "router_rank": student.router_rank,
        "reflectors_per_side": student.reflectors_per_side,
...
def artifact_student(
    ...
    student = LayerPrivateHouseholderTransport(
        ...
        reflectors_per_side=int(frozen["reflectors_per_side"]),
        router_q_seed=int(init["router_q_seed"]),
        router_p_seed=int(init["router_p_seed"]),
        tangent_v_seed=int(init["tangent_v_seed"]),
        input_reflector_seed=int(init["input_householder_seed"]),
        output_reflector_seed=int(init["output_householder_seed"]),
        ...
        reflector_epsilon=float(init["householder_denominator_epsilon"]),
    ...
    with torch.no_grad():
        for source_name, target_name in (
            ...
            ("input_reflectors", "input_reflectors"),
            ("output_reflectors", "output_reflectors"),
            ("anchors", "anchors"),
...
def student_function(
    student: LayerPrivateHouseholderTransport, mode: str
) -> Callable[
...
def gate_outcome(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "decision": (
            "PROMOTE_H65_TO_EXACT_SYSTEMS_GATE"
            if representation_pass
            else "REJECT_H65_LAYER_PRIVATE_HOUSEHOLDER_TRANSPORT"
        ),
    }
...
def main() -> None:
    ...
    if plan.get("schema_version") != PLAN_SCHEMA_VERSION:
        raise ValueError("unexpected H65 plan schema")
    if args.preflight:
    ...
    if accounting["total_checkpoint_payload_bytes"] != expected[
        ...
        raise AssertionError("H65 checkpoint-byte accounting mismatch")
    ...
    checkpoint_path = args.output / "layer_private_householder_checkpoint.pt"
    ...
    if result_bytes > int(plan["runtime_gates"]["maximum_result_directory_bytes"]):
        raise RuntimeError("H65 result directory exceeds frozen storage gate")
