#!/usr/bin/env python3
"""Gate a signed activation-conditioned span of complete procedural MLP atoms."""
...
PLAN_SCHEMA = "nanogpt_sparse_moe_signed_complete_atom_oracle_plan_v1"


class SignedCompleteAtoms(ConditionalCompleteAtoms):
    """Signed affine mixture of the same complete fixed-FHT atom family."""

    def _coefficients_and_jvp(
        self,
        values: torch.Tensor,
        ...
        *,
        ...
        if conditional:
            ...
            coefficient_delta = torch.einsum(
                "enr,ear->ena", router_base, router_weight
            ) / math.sqrt(float(self.router_feature_width))
            coefficient_jvp = torch.einsum(
                "enr,ear->ena", router_jvp, router_weight
            ) / math.sqrt(float(self.router_feature_width))
        else:
            coefficient_delta = torch.zeros(
                values.shape[0], values.shape[1], self.atoms,
                device=values.device, dtype=values.dtype,
            )
            coefficient_jvp = torch.zeros_like(coefficient_delta)
        coefficients = (
            1.0 / float(self.atoms)
            + self.router_bias[selected, None, :]
            + coefficient_delta
        )
        return coefficients, coefficient_jvp
    ...
    def function_and_jvp(
        ...
        *,
        ...
        if inputs.shape[0] != expected_experts or inputs.shape[-1] != self.input_width:
            raise ValueError("input shape disagrees with signed complete-atom operator")
        ...
        coefficients, coefficient_jvp = self._coefficients_and_jvp(
            values, tangent, selected=selected, conditional=conditional
        )
        atom_output = atom_output.permute(0, 2, 1, 3)
        atom_jvp = atom_jvp.permute(0, 2, 1, 3)
        output = (coefficients[..., None] * atom_output).sum(dim=2)
        output_jvp = (
            coefficients[..., None] * atom_jvp
            + coefficient_jvp[..., None] * atom_output
        ).sum(dim=2)
    ...
    def coefficient_diagnostics(
        ...
        *,
        ...
        coefficients, coefficient_jvp = self._coefficients_and_jvp(
            values, tangent, selected=slice(None), conditional=conditional
        )
        return {
            "coefficient_mean": float(coefficients.mean()),
            "coefficient_std": float(coefficients.std()),
            "coefficient_abs_mean": float(coefficients.abs().mean()),
            "coefficient_abs_max": float(coefficients.abs().max()),
            "coefficient_jvp_rms": float(coefficient_jvp.square().mean().sqrt()),
        }
    ...
    *,
    ...
    for _step in range(int(steps)):
        ...
        if not torch.isfinite(loss):
            raise RuntimeError("non-finite signed complete-atom objective")
        loss.backward()
        if any(
            parameter.grad is None or not torch.isfinite(parameter.grad).all()
            for parameter in parameters
        ):
            raise RuntimeError("non-finite or missing signed complete-atom gradient")
        gradient = float(
    ...
    diagnostics: dict[str, Any] = {
        ...
        "coefficient_bias_rms": float(
            module.router_bias.detach().square().mean().sqrt()
        ),
    }
    diagnostics.update(
        module.coefficient_diagnostics(
            live_inputs, directions, conditional=conditional
        )
    )
    return diagnostics
...
def make_module(plan: dict[str, Any], layer: int, device: str) -> SignedCompleteAtoms:
    source = plan["source"]
...
def validate_plan(plan: dict[str, Any], plan_path: Path) -> None:
    if plan.get("schema_version") != PLAN_SCHEMA:
        raise ValueError("signed complete-atom plan schema mismatch")
    ...
    if expected != int(candidate["total_coordinates_per_layer"]):
        raise ValueError("signed complete-atom accounting drift")
    ...
    if control_expected != int(plan["same_run_control"]["total_coordinates_per_layer"]):
        raise ValueError("signed static-control accounting drift")
...
def run_preflight(plan: dict[str, Any], device: str) -> dict[str, Any]:
    source = plan["source"]
    candidate = make_module(plan, 0, device)
    control = make_module(plan, 0, device)
    generator = torch.Generator(device="cpu").manual_seed(20261142)
    ...
    candidate_diag = fit_atoms(
        ...
        jvp_weight=float(fit["jvp_weight"]), probe_seed=20261143,
    )
    ...
    return {
        "schema_version": "nanogpt_sparse_moe_signed_complete_atom_preflight_v1",
        "device": device,
        "two_step_wall_seconds_candidate_plus_control": elapsed,
        "projected_full_protocol_seconds": (
            elapsed * (int(fit["steps"]) / 2.0) * 6.0
        ),
        "candidate_coordinate_count": candidate.compact_parameter_count(
            conditional=True
        ),
        "control_coordinate_count": control.compact_parameter_count(
            conditional=False
        ),
...
def main() -> None:
    ...
    positive_control = json.loads(
        (root / plan["sealed_controls"]["positive_simplex_result"]["path"])
    ...
    for bank_index, bank in enumerate(banks):
        ...
        for layer in layers:
            ...
            positive_layer = float(
                positive_control["summaries"][bank][str(layer)]["mixture_recovery"]
            )
            summaries[bank][str(layer)] = {
                ...
                "minimum_expert_jvp_recovery": min(
                    candidate_eval["expert_jvp_recovery"]
                ),
                ...
                "sealed_positive_simplex_recovery": positive_layer,
                "candidate_minus_sealed_positive_simplex_recovery": (
                    candidate_eval["mixture_recovery"] - positive_layer
                ),
            }
    ...
    for bank in banks:
        ...
        aggregate = {
            "mixture_recovery_mean": sum(
                float(row["mixture_recovery"]) for row in rows
            ) / len(rows),
            "mixture_recovery_minimum_layer": min(
                float(row["mixture_recovery"]) for row in rows
            ),
            "jvp_recovery_mean": sum(
                float(row["jvp_recovery"]) for row in rows
            ) / len(rows),
            ...
            "candidate_minus_static_control_recovery_mean": sum(
                float(row["candidate_minus_static_control_recovery"])
                for row in rows
            ) / len(rows),
            "candidate_minus_sealed_positive_simplex_recovery_mean": sum(
                float(row["candidate_minus_sealed_positive_simplex_recovery"])
                for row in rows
            ) / len(rows),
        ...
        bank_gates[bank] = {
            "mean_recovery_pass": aggregate["mixture_recovery_mean"] >= float(
                frozen["heldout_mixture_recovery_mean_min_each_bank"]
            ),
            "every_layer_pass": aggregate["mixture_recovery_minimum_layer"] >= float(
                frozen["heldout_mixture_recovery_every_layer_min_each_bank"]
            ),
            "every_expert_pass": aggregate["minimum_expert_recovery"] >= float(
                frozen["heldout_expert_recovery_min_each_bank"]
            ),
            "jvp_pass": aggregate["jvp_recovery_mean"] >= float(
                frozen["heldout_jvp_recovery_mean_min_each_bank"]
            ),
            "static_control_gain_pass": aggregate[
                "candidate_minus_static_control_recovery_mean"
            ] >= float(
                frozen["candidate_minus_static_control_recovery_mean_min_each_bank"]
            ),
            "positive_simplex_gain_pass": aggregate[
                "candidate_minus_sealed_positive_simplex_recovery_mean"
            ] >= float(
                frozen[
                    "candidate_minus_sealed_positive_simplex_recovery_mean_min_each_bank"
                ]
            ),
            "occupancy_pass": aggregate["minimum_discovery_assignments"] >= int(
    ...
    for bank in banks:
        bank_gates[bank]["action_agreement_pass"] = agreement_mean >= float(
    ...
    torch.save(
        {
            "schema_version": "nanogpt_sparse_moe_signed_complete_atom_coordinates_v1",
            "states": saved,
        },
        coordinates_path,
    )
    result = {
        "schema_version": "nanogpt_sparse_moe_signed_complete_atom_oracle_result_v1",
        "classification": (
            "SIGNED_COMPLETE_ATOM_REPRESENTABILITY_PASSES"
            if passed else "SIGNED_COMPLETE_ATOM_REPRESENTABILITY_REJECTED"
        ),
        "passed": passed,
        "identity": {
            ...
            "positive_simplex_result_sha256": file_sha256(
                root / plan["sealed_controls"]["positive_simplex_result"]["path"]
            ),
        },
        "execution": {
        ...
        "accounting": {
            "dense_paired_parameters": int(
                plan["candidate"]["dense_paired_parameters_all_layers"]
            ),
            "compact_coordinates": int(
            ...
            "signed_complete_atom_span": True,
        },
        "occupancy": occupancy,
