#!/usr/bin/env python3
"""Frozen H61 hard product-code value atlas plus affine-jet audit."""
...
SCHEMA_VERSION = "nanogpt_mlp_hard_productcode_value_affine_jet_functional_v1"
PLAN_SCHEMA_VERSION = (
    "nanogpt_mlp_hard_productcode_value_affine_jet_functional_plan_v1"
)
CANDIDATE_NAME = "learned_hard_productcode_value_affine_jet"
...
def deployment_accounting(
    *,
    ...
    router_code = 2 * total_codes * width
    private_code = layers * 2 * total_codes
    tangent = 2 * width * tangent_rank + layers * tangent_rank
    anchor_offset = layers * 2 * width
    total_values = router_code + private_code + tangent + anchor_offset
    ...
    return {
        ...
        "fp16_router_code_values": router_code,
        "fp16_private_code_values": private_code,
        "fp16_tangent_values": tangent,
        ...
        "extra_mlp_matmul_fraction": (
            width * total_codes + 2 * width * tangent_rank
        )
        / (8 * width * width),
...
class HardProductCodeValueAffineJet(torch.nn.Module):
    def __init__(
        self,
        ...
        *,
        ...
        self.codes_per_bank = int(codes_per_bank)
        self.total_codes = self.banks * self.codes_per_bank
        ...
        initial_router = deterministic_router(
            self.total_codes, width, router_seed
        )
        ...
        self.codebook = torch.nn.Parameter(
            torch.zeros(self.total_codes, width)
        )
        self.router_bias = torch.nn.Parameter(
            torch.full(
                (len(layers), self.total_codes), float(router_bias_initial)
            )
        )
        self.code_gain = torch.nn.Parameter(
            torch.full((len(layers), self.total_codes), float(code_gain_initial))
        )
    ...
    def _parts(self, row: int, mode: str) -> dict[str, torch.Tensor | bool]:
        ...
        if mode == "step_zero_parent":
            ...
            parts["router"] = self.initial_router
        elif mode == "single_bank_only":
            parts["single_bank"] = True
        elif mode != "full":
            raise ValueError(f"unknown H61 mode: {mode}")
        return parts

    def _assignment(self, logits: torch.Tensor) -> torch.Tensor:
        shaped = logits.reshape(-1, self.banks, self.codes_per_bank)
        index = shaped.argmax(dim=-1)
        hard = F.one_hot(index, self.codes_per_bank).to(shaped.dtype)
        if self.training:
            soft = torch.softmax(shaped / self.temperature, dim=-1)
    ...
    def forward_function(
        ...
        *,
        ...
        code_gain = p["code_gain"]
        tangent_u = p["tangent_u"]
        tangent_v = p["tangent_v"]
        tangent_coordinate = p["tangent_coordinate"]
        anchor = p["anchor"]
        offset = p["offset"]
        assert isinstance(router, torch.Tensor)
        assert isinstance(codebook, torch.Tensor)
        assert isinstance(router_bias, torch.Tensor)
        assert isinstance(code_gain, torch.Tensor)
        assert isinstance(tangent_u, torch.Tensor)
        assert isinstance(tangent_v, torch.Tensor)
        assert isinstance(tangent_coordinate, torch.Tensor)
        ...
        logits = base_output @ router.T + router_bias
        assignment = self._assignment(logits)
        if bool(p["single_bank"]):
            mask = torch.zeros_like(assignment)
            mask[:, 0, :] = assignment[:, 0, :]
            assignment = mask
        weighted = assignment.reshape(-1, self.total_codes) * code_gain
        value_correction = weighted @ codebook
        tangent_projection = (inputs - anchor) @ tangent_v
        tangent_correction = (
            tangent_projection * tangent_coordinate
        ) @ tangent_u.T
        ...
        tangent_action = (
            (directions @ tangent_v) * tangent_coordinate
        ) @ tangent_u.T
        # The deployed hard route is piecewise constant. Its exact local JVP
        # is zero away from measure-zero argmax boundaries. The STE is used
        # only for output-gradient optimization and never reported as a JVP.
        return output, base_action + tangent_action


def optimizer_for(
    student: HardProductCodeValueAffineJet, plan: dict[str, Any]
...
def fit_student(
    student: HardProductCodeValueAffineJet,
    bank: FunctionalBank,
    ...
    *,
    ...
    for iteration in range(iterations):
        ...
        if action is None:
            raise RuntimeError("H61 student JVP is missing")
        output_loss = (
...
def terminal_artifact(
    student: HardProductCodeValueAffineJet, accounting: dict[str, Any]
) -> dict[str, Any]:
    values = {
    ...
    return {
        "schema_version": "hard_productcode_value_affine_jet_checkpoint_v1",
        **values,
...
def artifact_student(
    ...
    student = HardProductCodeValueAffineJet(
        ...
        router_seed=int(init["router_seed"]),
...
def student_function(
    student: HardProductCodeValueAffineJet, mode: str
) -> Callable[
...
def gate_outcome(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "decision": (
            "PROMOTE_H61_TO_EXACT_SYSTEMS_GATE"
            if representation_pass
            else "REJECT_H61_HARD_PRODUCTCODE_VALUE_AFFINE_JET"
        ),
    }
...
def main() -> None:
    ...
    if plan.get("schema_version") != PLAN_SCHEMA_VERSION:
        raise ValueError("unexpected H61 plan schema")
    if args.preflight:
    ...
    if (
        ...
        raise AssertionError("H61 checkpoint-byte accounting mismatch")
    ...
    anchors = torch.stack(
        [
            banks_data["train"].inputs[layer].float().mean(dim=0)
            for layer in layers
        ]
    )
    ...
    checkpoint_path = args.output / "hard_productcode_value_affine_jet_checkpoint.pt"
    ...
    candidates = {
        ...
        "hard_value_only": student_function(fitted, "hard_value_only"),
        "global_code_coordinates": student_function(
            fitted, "global_code_coordinates"
        ),
        "initial_random_router": student_function(
            fitted, "initial_random_router"
        ),
        "single_bank_only": student_function(fitted, "single_bank_only"),
    ...
    metadata = {
        ...
        "limitations": [
            ...
            "Hard-routing JVP is the deployed piecewise-constant derivative, not the straight-through training derivative.",
    ...
    if result_bytes > int(plan["runtime_gates"]["maximum_result_directory_bytes"]):
        raise RuntimeError("H61 result directory exceeds frozen storage gate")
