#!/usr/bin/env python3
"""Replay c_proj spatial-capacity arms with constant full error feedback."""
...
class Arm:
    name: str
    second_pass_stages: int


ARMS = (
    Arm("hidden88_full_carry", 24),
    Arm("hidden112_full_carry", 48),
)
CONTROL = ARMS[0].name
...
def structured_step(
    ...
    *,
    ...
    residual = corrected
    for pass_index, stages in enumerate((64, second_pass_stages)):
...
def _score_rows(rows: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
...
def aggregate_rows(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    endpoint_ratios = {
        str(step): float(scores[f"{CANDIDATE}@{step}"]["aggregate_endpoint_recovery"])
        / max(
    ...
    terminal_wins = sum(
        float(terminal_candidate_cells[layer]["endpoint_recovery"])
        > float(terminal_control_cells[layer]["endpoint_recovery"])
        for layer in terminal_candidate_cells
    )
    feedback_ratio = float(candidate_terminal["maximum_feedback_fro"]) / max(
        float(control_terminal["maximum_feedback_fro"]), 1e-30
    )
    comparison = {
        "endpoint_recovery_ratios_by_step": endpoint_ratios,
        "all_score_steps_no_worse": all(value >= 1.0 for value in endpoint_ratios.values()),
        "terminal_endpoint_recovery_ratio": endpoint_ratios[str(terminal_step)],
        "terminal_feedback_ratio": feedback_ratio,
        "terminal_layers_won": terminal_wins,
        "required_terminal_layers_won": 4,
        "terminal_row_gram_no_worse": float(
            candidate_terminal["aggregate_row_gram_recovery"]
        )
        >= float(control_terminal["aggregate_row_gram_recovery"]),
...
def main() -> None:
    ...
    if plan.get("schema_version") != (
        "mai_124m_mlp_cproj_teacher_forced_capacity_carry_plan_v1"
    ):
        ...
        for layer, name in zip(layers, names, strict=True):
            ...
            for arm in ARMS:
                ...
                updated, new_feedback, recovery = structured_step(
                    ...
                    requested,
                    feedback[key],
                    second_pass_stages=arm.second_pass_stages,
                    learning_rate=lr,
    ...
    cells_path = args.output / "cproj_capacity_carry_cells.csv"
    aggregate_path = args.output / "cproj_capacity_carry_result.json"
    write_csv(cells_path, rows)
    aggregate_path.write_text(json.dumps(aggregate, indent=2, sort_keys=True) + "\n")
    script = Path(__file__).resolve()
    metadata = {
        "schema_version": "nanogpt_cproj_teacher_forced_capacity_carry_v1",
    ...
    metadata_path = args.output / "cproj_capacity_carry_metadata.json"
