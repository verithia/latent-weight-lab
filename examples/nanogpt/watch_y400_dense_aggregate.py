...
def send(chat_id: str, text: str) -> bool:
    ...
    try:
        # The invocation endpoint may wait while the bridge allocates an agent
        # run before acknowledging delivery.  A short timeout creates the worst
        # possible ambiguity: the message may already be visible while the
        # monitor retries it because ownership was never persisted locally.
