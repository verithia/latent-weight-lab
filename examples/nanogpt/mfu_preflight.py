...
def execution_provenance(config_path: Path, source: dict[str, Any]) -> dict[str, Any]:
    ...
    paths = (
        ...
        root / "examples/nanogpt/pair_vq_lloyd_cuda.py",
        root / "examples/nanogpt/muon_int8_lattice.py",
        root / "csrc/pair_vq_lloyd_ext.cpp",
        root / "csrc/pair_vq_lloyd_ext_cuda.cu",
    )
...
def make_preflight_config(
    ...
    *,
    ...
    if config.get("pair_vq_dense_shadow_replay"):
        ...
        if config.get("pair_vq_dense_shadow_lowbit_momentum_result"):
            config["pair_vq_dense_shadow_lowbit_momentum_result"] = str(
                temporary_out / "pair_vq_lowbit_momentum_preflight.json"
            )
    if config.get("pair_vq_minifloat_momentum_result"):
