...
def summarize_log(path: Path, active_params: int, tokens_per_iter: int, peak_tflops: float) -> dict:
    ...
    mfu = train_tflops / peak_tflops if peak_tflops else 0.0
    return {
        "log": str(path),
        ...
        "reference_bf16_peak_tflops": peak_tflops,
        "estimated_mfu": mfu,
        "peak_mib": peak_mib,
    }
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=float,
        required=True,
        help="Measured or documented BF16 tensor-core peak for the actual GPU; never assume H100.",
    )
    ...
    summaries = [summarize_log(Path(path), active, args.tokens_per_iter, args.peak_tflops) for path in args.logs]
