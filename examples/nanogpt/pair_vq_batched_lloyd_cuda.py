"""Native CUDA sufficient statistics for layer-batched scalar Lloyd fits."""
...
def _load_extension():
    ...
    try:
        ...
        _EXTENSION = cpp_extension.load(
            name="latent_weight_lab_pair_vq_batched_lloyd_ext_v1",
            sources=[
                str(root / "csrc" / "pair_vq_batched_lloyd_ext.cpp"),
                str(root / "csrc" / "pair_vq_batched_lloyd_ext_cuda.cu"),
            ],
...
def batched_lloyd_stats(
    values: torch.Tensor,
    midpoints: torch.Tensor,
    *,
    level_count: int,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Compute disjoint BxK sums/counts without intermediate assignments."""
    if values.ndim != 2 or midpoints.ndim != 2:
        raise ValueError("batched Lloyd inputs must be matrices")
    if not values.is_contiguous() or not midpoints.is_contiguous():
        raise ValueError("batched Lloyd inputs must be contiguous")
    if values.dtype != torch.float32 or midpoints.dtype != torch.float32:
        raise ValueError("batched Lloyd inputs must be float32")
    if values.shape[0] != midpoints.shape[0]:
        raise ValueError("batched Lloyd row counts must match")
    if midpoints.shape[1] != int(level_count) - 1:
        raise ValueError("batched Lloyd midpoint count does not match levels")
    if not values.is_cuda or not midpoints.is_cuda:
        raise ValueError("batched fused Lloyd oracle requires CUDA")
    extension = _load_extension()
    if extension is None:
        raise RuntimeError("batched Lloyd CUDA extension failed to load") from _EXTENSION_ERROR
