#!/usr/bin/env python3
"""Frozen H62 factored-router expanded hard product-code functional audit."""
...
SCHEMA_VERSION = "nanogpt_mlp_factored_router_expanded_productcode_functional_v1"
PLAN_SCHEMA_VERSION = (
    "nanogpt_mlp_factored_router_expanded_productcode_functional_plan_v1"
)
CANDIDATE_NAME = "learned_factored_router_expanded_productcode_jet"
...
def deployment_accounting(
    *,
    ...
    width: int = 768,
    banks: int = 3,
    codes_per_bank: int = 97,
    router_rank: int = 64,
    tangent_rank: int = 96,
) -> dict[str, int | float]:
    total_codes = banks * codes_per_bank
    codebook = total_codes * width
    ...
    total_values = (
        codebook + factored_router + private_code + tangent + anchor_offset
    )
...
class FactoredRouterExpandedProductCode(HardProductCodeValueAffineJet):
    def __init__(
        self,
        ...
        *,
        ...
        super().__init__(
            ...
            tangent_initial=tangent_initial,
            offset_initial=offset_initial,
        )
        del self.router
        del self.initial_router
        self.router_rank = int(router_rank)
        initial_q = deterministic_orthonormal(
            self.width, self.router_rank, router_q_seed
        )
        initial_p = deterministic_router(
            self.total_codes, self.router_rank, router_p_seed
        )
    ...
    def _parts(self, row: int, mode: str) -> dict[str, torch.Tensor | bool]:
        ...
        if mode == "step_zero_parent":
            ...
            raise ValueError(f"unknown H62 mode: {mode}")
        return parts
    ...
    def forward_function(
        ...
        *,
        ...
        if not all(isinstance(value, torch.Tensor) for value in tensors.values()):
            raise TypeError("H62 tensor part is not a tensor")
        ...
        tangent_coordinate = tensors["tangent_coordinate"]
...
def optimizer_for(
    student: FactoredRouterExpandedProductCode, plan: dict[str, Any]
...
def fit_student(
    student: FactoredRouterExpandedProductCode,
    bank: FunctionalBank,
    ...
    *,
    ...
    for iteration in range(iterations):
        ...
        if action is None:
            raise RuntimeError("H62 student JVP is missing")
        output_loss = (
...
def terminal_artifact(
    student: FactoredRouterExpandedProductCode, accounting: dict[str, Any]
) -> dict[str, Any]:
    values = {
    ...
    return {
        "schema_version": "factored_router_expanded_productcode_checkpoint_v1",
        **values,
...
def artifact_student(
    ...
    plan: dict[str, Any],
    device: str,
) -> FactoredRouterExpandedProductCode:
...
def student_function(
    student: FactoredRouterExpandedProductCode, mode: str
) -> Callable[
...
def gate_outcome(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "decision": (
            "PROMOTE_H62_TO_EXACT_SYSTEMS_GATE"
            if representation_pass
            else "REJECT_H62_FACTORED_ROUTER_EXPANDED_PRODUCTCODE"
        ),
    }
...
def main() -> None:
    ...
    if plan.get("schema_version") != PLAN_SCHEMA_VERSION:
        raise ValueError("unexpected H62 plan schema")
    if args.preflight:
    ...
    if accounting["total_checkpoint_payload_bytes"] != expected[
        ...
        raise AssertionError("H62 checkpoint-byte accounting mismatch")
    ...
    checkpoint_path = args.output / "factored_router_expanded_productcode_checkpoint.pt"
    ...
    if result_bytes > int(plan["runtime_gates"]["maximum_result_directory_bytes"]):
        raise RuntimeError("H62 result directory exceeds frozen storage gate")
