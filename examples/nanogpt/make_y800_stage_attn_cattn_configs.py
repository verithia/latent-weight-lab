...
BASE = {
    ...
    "dtype": "bfloat16",
    "compile": False,
    "block_fht_targets": ["attn.c_attn", "attn.c_proj", "mlp.c_fc"],
    "block_fht_latent_ratio": 0.08,
    "block_fht_latent_ratios": {"attn.c_attn": 0.08, "attn.c_proj": 0.08, "mlp.c_fc": 0.10},
...
def main() -> None:
    specs = [
        (
            "y800_stage4_100m_01_cproj_cfc_l10_lr32_cattn_l8",
            {"block_fht_latent_ratios": {"attn.c_attn": 0.08, "attn.c_proj": 0.08, "mlp.c_fc": 0.10}},
        ),
        (
            "y800_stage4_100m_02_cproj_cfc_l10_lr32_cattn_l10",
            {"block_fht_latent_ratios": {"attn.c_attn": 0.10, "attn.c_proj": 0.08, "mlp.c_fc": 0.10}},
        ),
        (
            "y800_stage4_100m_03_cproj_cfc_l10_lr32_cattn_l12",
            {"block_fht_latent_ratios": {"attn.c_attn": 0.12, "attn.c_proj": 0.08, "mlp.c_fc": 0.10}},
        ),
        (
            "y800_stage4_100m_04_cproj_cfc_l10_lr32_cattn_gain",
            {
                "block_fht_latent_ratios": {"attn.c_attn": 0.10, "attn.c_proj": 0.08, "mlp.c_fc": 0.10},
                "block_fht_output_gain_targets": ["attn.c_attn"],
                "block_fht_cache_weights": False,
            },
        ),
    ]
    tasks = [(name, write(name, **updates)) for name, updates in specs]
    queue_path = CONFIG_DIR / "y800_stage_attn_cattn_queue.tsv"
