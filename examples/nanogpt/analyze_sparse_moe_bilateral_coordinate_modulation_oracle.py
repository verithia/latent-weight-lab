#!/usr/bin/env python3
"""Gate bilateral hidden/output coordinate modulation of complete MLP atoms."""
...
from examples.nanogpt.analyze_sparse_moe_residual_coordinate_modulation_oracle import (
    ...
    fit_atoms as fit_output_modulated_atoms,
)
...
PLAN_SCHEMA = "nanogpt_sparse_moe_bilateral_coordinate_modulation_oracle_plan_v1"
...
def coordinate_count(
    *, experts: int, atoms: int, input_width: int, hidden_width: int,
    ...
    if conditional:
        count += experts * atoms * hidden_width
    return count


class BilateralCoordinateModulatedAtoms(ResidualCoordinateModulatedAtoms):
    """Output modulation plus optional pre-GELU hidden-coordinate modulation."""

    def __init__(self, **kwargs: Any) -> None:
        super().__init__(**kwargs)
        reference = torch.empty(1, device=self.hidden_bias.device, dtype=torch.float32)
        layer_seed = int(kwargs["seed"]) + 1009 * int(kwargs["layer"])
        hidden_signs = []
        for expert in range(self.experts):
            hidden_signs.append(
                torch.stack(
                    [
                        signs_for(
                            reference, expert, 3 * self.atoms + atom,
        ...
        self.register_buffer(
            "hidden_modulation_signs", torch.stack(hidden_signs).float()
        )
        self.hidden_modulation = torch.nn.Parameter(
    ...
    def trainable_parameters(self, *, conditional: bool) -> list[torch.nn.Parameter]:
        parameters = [
            ...
            self.hidden_bias,
            self.residual_modulation,
        ]
        if conditional:
            parameters.append(self.hidden_modulation)
        return parameters
    ...
    def function_and_jvp(
        ...
        if inputs.shape[0] != expected_experts or inputs.shape[-1] != self.input_width:
            raise ValueError("input shape disagrees with bilateral modulation operator")
        ...
        base = normalized_fht_last_dim(
            values[:, None, :, :] * self.input_signs[selected, :, None, :]
        )[..., : self.hidden_width]
        base_jvp = normalized_fht_last_dim(
            tangent[:, None, :, :] * self.input_signs[selected, :, None, :]
        )[..., : self.hidden_width]
        hidden_gain = 1.0 + self.hidden_gain_delta[selected, :, None, :]
        if conditional:
            raw_hidden_feature = normalized_fht_last_dim(
                values[:, None, :, :]
                * self.hidden_modulation_signs[selected, :, None, :]
            )[..., : self.hidden_width] * self.modulation_feature_scale
            raw_hidden_feature_jvp = normalized_fht_last_dim(
                tangent[:, None, :, :]
                * self.hidden_modulation_signs[selected, :, None, :]
            )[..., : self.hidden_width] * self.modulation_feature_scale
            hidden_feature = torch.tanh(raw_hidden_feature)
            hidden_feature_jvp = (
                1.0 - hidden_feature.square()
            ) * raw_hidden_feature_jvp
            hidden_modulation = self.hidden_modulation[selected, :, None, :]
            hidden_scale = hidden_gain + hidden_modulation * hidden_feature
            hidden_scale_jvp = hidden_modulation * hidden_feature_jvp
        else:
            hidden_scale = hidden_gain
            hidden_scale_jvp = torch.zeros_like(base)
        pre = (
            self.c_fc_scale * base * hidden_scale
            + self.hidden_bias[selected, None, None, :]
        )
        pre_jvp = self.c_fc_scale * (
            base_jvp * hidden_scale + base * hidden_scale_jvp
        )
        hidden = F.gelu(pre)
        hidden_jvp = (
        ...
        raw_output_feature = normalized_fht_last_dim(
            values[:, None, :, :] * self.modulation_signs[selected, :, None, :]
        )[..., : self.input_width] * self.modulation_feature_scale
        raw_output_feature_jvp = normalized_fht_last_dim(
            tangent[:, None, :, :] * self.modulation_signs[selected, :, None, :]
        )[..., : self.input_width] * self.modulation_feature_scale
        output_feature = torch.tanh(raw_output_feature)
        output_feature_jvp = (
            1.0 - output_feature.square()
        ) * raw_output_feature_jvp
        output_modulation = self.residual_modulation[selected, :, None, :]
        output_scale = 1.0 + output_modulation * output_feature
        output_scale_jvp = output_modulation * output_feature_jvp
        output = (output_scale * atom_output).mean(dim=1)
        output_jvp = (
            output_scale * atom_jvp + output_scale_jvp * atom_output
        ).mean(dim=1)
        return output, output_jvp


def fit_atoms(
    module: BilateralCoordinateModulatedAtoms,
    inputs: torch.Tensor,
    dense_c_fc: torch.Tensor,
    dense_c_proj: torch.Tensor,
    **kwargs: Any,
) -> dict[str, Any]:
    diagnostics = fit_output_modulated_atoms(
        module, inputs, dense_c_fc, dense_c_proj, **kwargs
    )
    diagnostics["hidden_modulation_rms"] = float(
        module.hidden_modulation.detach().square().mean().sqrt()
    )
    return diagnostics


def make_module(plan: dict[str, Any], layer: int, device: str) -> BilateralCoordinateModulatedAtoms:
...
def validate_plan(plan: dict[str, Any], plan_path: Path) -> None:
    if plan.get("schema_version") != PLAN_SCHEMA:
        raise ValueError("bilateral-coordinate plan schema mismatch")
...
def run_preflight(plan: dict[str, Any], device: str) -> dict[str, Any]:
    ...
    generator = torch.Generator(device="cpu").manual_seed(20261162)
    ...
    return {
        "schema_version": "nanogpt_sparse_moe_bilateral_coordinate_modulation_preflight_v1",
...
def main() -> None:
    ...
    root = Path(__file__).resolve().parents[2]
    output_control = json.loads(
        (root / plan["sealed_controls"]["output_coordinate_result"]["path"]).read_text()
    )
    fit = plan["fit_protocol"]
    ...
    for bank_index, bank in enumerate(banks):
        ...
        for layer in layers:
            ...
            sampled, counts = route_and_sample(
                ...
                seed=20261164 + 1009 * bank_index + 17 * layer,
            )
            ...
            candidate_diag = fit_atoms(
                ...
                jvp_weight=float(fit["jvp_weight"]), probe_seed=20261165 + 1009 * bank_index + 17 * layer,
            )
            control_diag = fit_atoms(
            ...
            candidate_eval = routed_evaluation(
                ...
                outer_top_k=int(source["outer_moe_top_k"]), probe_seed=20261166 + 17 * layer,
            )
            ...
            sealed_layer = float(output_control["summaries"][bank][str(layer)]["mixture_recovery"])
            summaries[bank][str(layer)] = {
                ...
                "output_only_control_recovery": control_eval["mixture_recovery"],
                "candidate_minus_output_only_control_recovery": candidate_eval["mixture_recovery"] - control_eval["mixture_recovery"],
                "sealed_output_coordinate_recovery": sealed_layer,
                "candidate_minus_sealed_output_coordinate_recovery": candidate_eval["mixture_recovery"] - sealed_layer,
            }
    ...
    for bank in banks:
        ...
        aggregate = {
            ...
            "candidate_minus_output_only_control_recovery_mean": sum(float(r["candidate_minus_output_only_control_recovery"]) for r in rows) / len(rows),
            "candidate_minus_sealed_output_coordinate_recovery_mean": sum(float(r["candidate_minus_sealed_output_coordinate_recovery"]) for r in rows) / len(rows),
        ...
        bank_gates[bank] = {
            ...
            "output_only_control_gain_pass": aggregate["candidate_minus_output_only_control_recovery_mean"] >= float(frozen["candidate_minus_output_only_control_recovery_mean_min_each_bank"]),
            "sealed_output_coordinate_gain_pass": aggregate["candidate_minus_sealed_output_coordinate_recovery_mean"] >= float(frozen["candidate_minus_sealed_output_coordinate_recovery_mean_min_each_bank"]),
    ...
    torch.save({"schema_version": "nanogpt_sparse_moe_bilateral_coordinate_modulation_coordinates_v1", "states": saved}, coordinates_path)
    result = {
        "schema_version": "nanogpt_sparse_moe_bilateral_coordinate_modulation_oracle_result_v1",
        "classification": "BILATERAL_COORDINATE_MODULATION_REPRESENTABILITY_PASSES" if passed else "BILATERAL_COORDINATE_MODULATION_REPRESENTABILITY_REJECTED",
        "passed": passed,
        "identity": {
            ...
            "output_coordinate_result_sha256": file_sha256(root / plan["sealed_controls"]["output_coordinate_result"]["path"]),
        },
        "execution": {
        ...
        "accounting": {
            ...
            "fixed_full_matrix_storage": False, "bilateral_coordinate_modulation": True,
        },
