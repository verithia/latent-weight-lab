...
PLAN = REPO_ROOT / "examples/nanogpt/configs/selection_artifacts/124m_mlp_token_router_residual_carrier_functional_plan.json"


def test_direct_entrypoint_resolves_repository_package() -> None:
    script = Path(__file__).with_name(
        "analyze_mlp_token_router_residual_carrier_functional.py"
    )
...
def test_exact_h57_accounting_matches_plan() -> None:
...
def test_deterministic_orthonormal_frame() -> None:
    frame = deterministic_orthonormal(32, 7, 19)
    torch.testing.assert_close(frame.T @ frame, torch.eye(7), atol=1e-5, rtol=1e-5)
    assert torch.equal(frame, deterministic_orthonormal(32, 7, 19))


def tiny_student() -> TokenRouterResidualCarrier:
    generator = torch.Generator().manual_seed(7)
    detector = {0: torch.randn(8, 4, generator=generator)}
    write = {0: torch.randn(8, 4, generator=generator)}
    return TokenRouterResidualCarrier(
        detector,
        write,
        [0],
        group_width=2,
...
def test_analytic_jvp_matches_autograd() -> None:
    ...
    _output, automatic = torch.autograd.functional.jvp(
        lambda value: student.forward_function(0, value, None)[0],
        inputs,
        direction,
    )
    torch.testing.assert_close(output, _output)
    torch.testing.assert_close(analytic, automatic, atol=2e-5, rtol=2e-5)


def test_compact_artifact_and_gradients() -> None:
    student = tiny_student()
    ...
    accounting = deployment_accounting(
        layers=1, width=4, hidden=8, group_width=2, carrier_rank=2
    )
