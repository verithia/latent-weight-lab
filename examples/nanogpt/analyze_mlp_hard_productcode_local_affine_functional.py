#!/usr/bin/env python3
"""Frozen H63 hard product-code local affine MLP functional audit."""
...
SCHEMA_VERSION = "nanogpt_mlp_hard_productcode_local_affine_functional_v1"
PLAN_SCHEMA_VERSION = "nanogpt_mlp_hard_productcode_local_affine_functional_plan_v1"
CANDIDATE_NAME = "learned_hard_productcode_local_affine_atlas"
...
def deployment_accounting(
    *,
    ...
    slope_coordinates = layers * tangent_rank + total_codes * tangent_rank
    ...
    return {
        ...
        "fp16_slope_coordinate_values": slope_coordinates,
...
class HardProductCodeLocalAffine(HardProductCodeValueAffineJet):
    def __init__(
        self,
        ...
        *,
        ...
        temperature: float,
        layer_slope_initial: float,
        code_slope_initial: float,
        ...
        super().__init__(
            ...
            tangent_initial=layer_slope_initial,
            offset_initial=offset_initial,
        )
        self.code_slope = torch.nn.Parameter(
            torch.full(
                (self.total_codes, self.tangent_rank), float(code_slope_initial)
            )
        )
    ...
    def _parts(self, row: int, mode: str) -> dict[str, torch.Tensor | bool]:
        parts: dict[str, torch.Tensor | bool] = {
            ...
            "layer_slope": self.tangent_coordinate[row],
            "code_slope": self.code_slope,
        ...
        if mode == "step_zero_parent":
        ...
        elif mode == "layer_affine_only":
            parts["codebook"] = torch.zeros_like(self.codebook)
            parts["code_slope"] = torch.zeros_like(self.code_slope)
            parts["offset"] = torch.zeros_like(self.output_offset[row])
        elif mode == "piecewise_slope_only":
            parts["codebook"] = torch.zeros_like(self.codebook)
            parts["offset"] = torch.zeros_like(self.output_offset[row])
        elif mode == "hard_value_only":
            parts["tangent_u"] = torch.zeros_like(self.tangent_u)
            parts["offset"] = torch.zeros_like(self.output_offset[row])
        elif mode == "no_code_slope":
            parts["code_slope"] = torch.zeros_like(self.code_slope)
            ...
            raise ValueError(f"unknown H63 mode: {mode}")
        return parts
    ...
    def forward_function(
        ...
        *,
        ...
        tensors = {
            key: value for key, value in p.items() if key != "single_bank"
        }
        if not all(isinstance(value, torch.Tensor) for value in tensors.values()):
            raise TypeError("H63 tensor part is not a tensor")
        ...
        layer_slope = tensors["layer_slope"]
        code_slope = tensors["code_slope"]
        ...
        local_slope = layer_slope + flat_assignment @ code_slope
        tangent_projection = (inputs - anchor) @ tangent_v
        tangent_correction = (tangent_projection * local_slope) @ tangent_u.T
        ...
        tangent_action = (
            (directions @ tangent_v) * local_slope
        ) @ tangent_u.T
        return output, base_action + tangent_action


def optimizer_for(
    student: HardProductCodeLocalAffine, plan: dict[str, Any]
...
def fit_student(
    student: HardProductCodeLocalAffine,
    bank: FunctionalBank,
    ...
    *,
    ...
    for iteration in range(iterations):
        ...
        if action is None:
            raise RuntimeError("H63 student JVP is missing")
        output_loss = (
...
def terminal_artifact(
    student: HardProductCodeLocalAffine, accounting: dict[str, Any]
) -> dict[str, Any]:
    values = {
        ...
        "layer_slope": student.tangent_coordinate.detach().to(torch.float16).cpu(),
        "code_slope": student.code_slope.detach().to(torch.float16).cpu(),
    ...
    return {
        "schema_version": "hard_productcode_local_affine_checkpoint_v1",
        **values,
...
def artifact_student(
    ...
    student = HardProductCodeLocalAffine(
        ...
        layer_slope_initial=float(init["layer_slope_coordinate"]),
        code_slope_initial=float(init["code_slope_coordinate"]),
    ...
    with torch.no_grad():
        for source_name, target_name in (
            ...
            ("tangent_u", "tangent_u"),
            ("tangent_v", "tangent_v"),
            ("layer_slope", "tangent_coordinate"),
            ("code_slope", "code_slope"),
            ("anchors", "anchors"),
...
def student_function(
    student: HardProductCodeLocalAffine, mode: str
) -> Callable[
...
def gate_outcome(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "decision": (
            "PROMOTE_H63_TO_EXACT_SYSTEMS_GATE"
            if representation_pass
            else "REJECT_H63_HARD_PRODUCTCODE_LOCAL_AFFINE"
        ),
    }
...
def main() -> None:
    ...
    if plan.get("schema_version") != PLAN_SCHEMA_VERSION:
        raise ValueError("unexpected H63 plan schema")
    if args.preflight:
    ...
    if accounting["total_checkpoint_payload_bytes"] != expected[
        ...
        raise AssertionError("H63 checkpoint-byte accounting mismatch")
    ...
    checkpoint_path = args.output / "hard_productcode_local_affine_checkpoint.pt"
    ...
    candidates = {
        ...
        "layer_affine_only": student_function(fitted, "layer_affine_only"),
        "piecewise_slope_only": student_function(
            fitted, "piecewise_slope_only"
        ),
        "hard_value_only": student_function(fitted, "hard_value_only"),
        "no_code_slope": student_function(fitted, "no_code_slope"),
    ...
    if result_bytes > int(plan["runtime_gates"]["maximum_result_directory_bytes"]):
        raise RuntimeError("H63 result directory exceeds frozen storage gate")
