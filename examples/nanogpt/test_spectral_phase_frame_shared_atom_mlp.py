...
def tiny_config(**updates) -> GPTConfig:
    values = dict(
        ...
        n_embd=16,
        bias=False,
        compact_native_mlp="shared_spectral_phase_frame353",
        compact_native_mlp_shared_width=8,
        compact_native_mlp_seed=41,
    )
...
def production_config(**updates) -> GPTConfig:
    values = dict(
        ...
        bias=False,
        compact_native_mlp="shared_spectral_phase_frame353",
        compact_native_mlp_shared_width=353,
        compact_native_mlp_seed=20260901,
    )
    values.update(updates)
    return GPTConfig(**values)


def test_frozen_production_state_single_bank_and_no_stored_basis() -> None:
    model = GPT(production_config())
    bank = model.shared_compact_mlp_spectral_phase_bank
    ...
    dense = 12 * 8 * 768 * 768
    assert shared == 542_208
    assert local == 22_644
    assert shared + local == 564_852
    assert abs((shared + local) / dense - 0.009975645277235243) < 1e-15
    keys = set(model.state_dict())
    assert "shared_compact_mlp_spectral_phase_bank.input_weight" in keys
    assert "shared_compact_mlp_spectral_phase_bank.output_weight" in keys
    assert not any("mlp._shared_bank" in key for key in keys)
    assert not any(
        "frame_permutation" in key
        or "frame_sign" in key
...
def test_zero_phase_is_identity_and_nonzero_frame_is_orthogonal() -> None:
    torch.manual_seed(13)
    model = GPT(tiny_config(n_layer=1)).eval()
    mlp = model.transformer.h[0].mlp
    x = torch.randn(7, 16)
    torch.testing.assert_close(
        mlp._apply_spectral_frame(x, mlp.input_phase),
        x,
        rtol=2e-6,
        atol=2e-6,
    )
    with torch.no_grad():
        mlp.input_phase.copy_(torch.linspace(-0.7, 0.9, mlp.phase_bins))
    transformed = mlp._apply_spectral_frame(x, mlp.input_phase)
    restored = mlp._apply_spectral_frame(
        transformed,
        mlp.input_phase,
        inverse=True,
    )
    torch.testing.assert_close(restored, x, rtol=3e-6, atol=3e-6)
    torch.testing.assert_close(
    ...
    basis = torch.eye(16)
    delta = mlp._apply_spectral_frame(basis, mlp.input_phase) - basis
    # DC and Nyquist are fixed; every other real Fourier direction moves.
    assert int(torch.linalg.matrix_rank(delta, atol=1e-5)) == 14
...
def test_folded_forward_matches_activation_formula_and_gradients() -> None:
    ...
    mlp = model.transformer.h[0].mlp
    with torch.no_grad():
        mlp.input_phase.copy_(torch.linspace(-0.4, 0.5, mlp.phase_bins))
        mlp.output_phase.copy_(torch.linspace(0.6, -0.3, mlp.phase_bins))
    x = torch.randn(2, 3, 16)
    actual = mlp(x)
    reference = mlp.activation_space_forward(x)
    torch.testing.assert_close(actual, reference, rtol=3e-5, atol=4e-6)
...
def test_forward_backward_freeze_and_phase_gradients_noncollapsed() -> None:
    ...
    loss.backward()
    compact = [
        *model.shared_compact_mlp_spectral_phase_bank.parameters(),
        *(
            parameter
            for block in model.transformer.h
            for parameter in block.mlp.parameters()
    ...
    for name in ("input_phase", "output_phase"):
        first_gradient = getattr(first, name).grad
        second_gradient = getattr(second, name).grad
...
def test_checkpoint_round_trip_is_exact() -> None:
    ...
    with torch.no_grad():
        first.transformer.h[0].mlp.input_phase.add_(0.1)
...
def test_shared_matrices_use_muon_and_phase_coordinates_use_adamw() -> None:
    model = GPT(production_config())
    ...
    for block in model.transformer.h:
        assert all(
            ...
            for parameter in (
                block.mlp.input_phase,
                block.mlp.output_phase,
