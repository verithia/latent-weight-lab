#!/usr/bin/env python3
"""Fail-closed validation for a functional-shear MFU/stability preflight log."""
...
PREFIX = "muon_matched_givens_refresh "
LOSS_TOKEN = r"(?:nan|[-+]?inf|[-+]?(?:\d+(?:\.\d*)?|\.\d+)(?:e[-+]?\d+)?)"
...
def parse_functional_rows(text: str) -> list[dict[str, Any]]:
    rows = []
    for line in text.splitlines():
        ...
        try:
            ...
            continue
        if isinstance(value, dict) and value.get("optimizer") == "muon_functional_shear":
            rows.append(value)
    return rows
...
def parse_losses(text: str) -> list[float]:
    ...
    for line in text.splitlines():
        iteration = re.match(
            rf"^iter\s+\d+:\s+loss\s+({LOSS_TOKEN})(?:,|\s|$)",
            line,
            flags=re.IGNORECASE,
        )
        if iteration:
            losses.append(float(iteration.group(1)))
            continue
        evaluation = re.match(
            rf"^step\s+\d+:\s+train loss\s+({LOSS_TOKEN}),\s+val loss\s+({LOSS_TOKEN})(?:\s|$)",
            line,
            flags=re.IGNORECASE,
        )
        if evaluation:
            losses.extend((float(evaluation.group(1)), float(evaluation.group(2))))
    return losses
...
def validate(
    ...
    *,
    ...
    observed_coordinates = {
        (row.get("step"), row.get("layer")) for row in rows
    }
    ...
    for row in rows:
        ...
        if row.get("functional_fit_condition_projection_active") is not True:
            failures.append("internal functional-fit condition limiter was not exercised")
    ...
    if internal_limiter_rows != len(rows):
        failures.append("the internal condition limiter was not active on every row")
    if fallback_rows:
    ...
    return {
        "schema_version": "functional_shear_stability_validation_v1",
        ...
        "registered_contract": {
            ...
            "require_internal_limiter_every_row": True,
