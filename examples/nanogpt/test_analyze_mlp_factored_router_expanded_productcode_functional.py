...
from examples.nanogpt.analyze_mlp_factored_router_expanded_productcode_functional import (
...
PLAN = REPO_ROOT / "examples/nanogpt/configs/selection_artifacts/124m_mlp_factored_router_expanded_productcode_functional_plan.json"


def test_direct_entrypoint_resolves_repository_package() -> None:
    script = Path(__file__).with_name(
        "analyze_mlp_factored_router_expanded_productcode_functional.py"
    )
...
def test_exact_h62_accounting_matches_plan() -> None:
    ...
    assert accounting["extra_mlp_matmul_fraction"] < 0.05


def tiny_student() -> FactoredRouterExpandedProductCode:
    ...
    return FactoredRouterExpandedProductCode(
        detector,
        write,
        [0],
        anchors,
...
def test_factored_router_shapes_and_hard_jvp() -> None:
    student = tiny_student()
    assert tuple(student.router_q.shape) == (4, 2)
    assert tuple(student.router_p.shape) == (6, 2)
    student.eval()
    with torch.no_grad():
...
def test_hard_value_jvp_identity_and_router_gradient() -> None:
    student = tiny_student()
    with torch.no_grad():
        student.codebook.normal_()
    ...
    assert student.router_q.grad is not None
    assert float(student.router_q.grad.norm()) > 0
    assert student.router_p.grad is not None
    accounting = deployment_accounting(
        layers=1,
        width=4,
        banks=2,
