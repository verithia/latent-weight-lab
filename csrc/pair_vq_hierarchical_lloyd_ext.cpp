...
std::vector<torch::Tensor> pair_vq_hierarchical_lloyd_stats_fp64(
    torch::Tensor values,
    torch::Tensor midpoints,
    int64_t level_count) {
  TORCH_CHECK(values.is_cuda() && midpoints.is_cuda(),
  ...
  TORCH_CHECK(midpoints.size(1) == level_count - 1,
              "hierarchical Lloyd midpoint count does not match levels");
  return pair_vq_hierarchical_lloyd_stats_fp64_cuda(
      values, midpoints, level_count);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("stats_fp64", &pair_vq_hierarchical_lloyd_stats_fp64,
        "FP64-accumulated hierarchical Pair-VQ Lloyd statistics (CUDA)");
}
