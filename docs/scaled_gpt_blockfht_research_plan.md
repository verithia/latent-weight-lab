...
Current blocker: Y400 `/root/userdata/MappingNetworks` is already about `63G`. A 20B-token uint16 dataset needs roughly `40GB` for `train.bin` plus validation/cache overhead, so `prepare_y400_finewebedu.sh` must not be launched until space is freed or the workspace cap changes.
